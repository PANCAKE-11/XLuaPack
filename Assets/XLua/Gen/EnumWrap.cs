...
namespace XLua.CSObjectWrap
{
    ...
    public class ItemTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
			...
			Utils.BeginClassRegister(typeof(ItemType), L, null, 6, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "拉面", ItemType.拉面);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "宝石", ItemType.宝石);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "卷轴", ItemType.卷轴);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "one", ItemType.one);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "two", ItemType.two);
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
			    ...
			    if (LuaAPI.xlua_is_eq_str(L, 1, "拉面"))
                {
                    translator.PushItemType(L, ItemType.拉面);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "宝石"))
                {
                    translator.PushItemType(L, ItemType.宝石);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "卷轴"))
                {
                    translator.PushItemType(L, ItemType.卷轴);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "one"))
                {
                    translator.PushItemType(L, ItemType.one);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "two"))
                {
                    translator.PushItemType(L, ItemType.two);
                }
				else
                {
